...
fn generate_permutations(max_length: usize, valid_chars: &[char], print_length: usize, target_selector: [u8; 4]) {
    ...
    (32..=max_length.min(64)).into_par_iter().for_each(|length| {
        ...
        for n in 0..valid_chars_len.pow(length as u32) {
            ...
            let current_selector = generate_function_signature(&function_name, "(address,address,uint256,uint256,uint8,bytes32,bytes32)");
...
fn main() {
    ...
    let target_name = "permit";
    let target_args = "(address,address,uint256,uint256,uint8,bytes32,bytes32)";
